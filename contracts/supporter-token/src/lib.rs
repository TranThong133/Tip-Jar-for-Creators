...
use soroban_sdk::{
    ...
    Address, Env, MuxedAddress, String, Symbol,
};
...
#[contract]
pub struct SupporterToken;

#[contractimpl]
impl SupporterToken {
    /// Constructor — sets admin and metadata.
    ...
    /// Admin-only: mint new tokens to `to`.
    /// Signature matches the Stellar Asset Contract's `mint` so callers can use either.
    pub fn mint(env: Env, to: Address, amount: i128) {
        ...
        env.events().publish((Symbol::new(&env, "mint"), admin, to.clone()), amount);
    }

    /// Admin-only: transfer the admin role.
...
#[contractimpl]
impl TokenInterface for SupporterToken {
    ...
    fn transfer(env: Env, from: Address, to: MuxedAddress, amount: i128) {
        ...
        env.events().publish((Symbol::new(&env, "transfer"), from, to_addr), amount);
    }
